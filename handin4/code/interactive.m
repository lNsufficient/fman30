...
GUI.SPEED = 1./abs(blurredIm-blurredIntensityatseed);
GUI.SPEED = GUI.SPEED - min(min(GUI.SPEED));
GUI.SPEED = GUI.SPEED*GUI.Slider3^2;
TOL = 10^(-GUI.Slider1/100);
...
blurredLap = imfilter(GUI.IM, gauss_filter_lap, 'same');
blurredLap = imfilter(blurredLap, laplace_filter, 'same');
blurredLap = 1./(1 + exp(GUI.Slider3*blurredLap));
GUI.SPEED = GUI.SPEED + 0.2*blurredLap;
